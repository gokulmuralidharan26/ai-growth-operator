...
function generateMemoMarkdown(
  ...
  const lines: string[] = [
    `# ${snapshot.brandName} — Weekly Growth Memo`,
    `**Period:** ${snapshot.timeWindow} ending ${date}`,
    `**Industry:** ${snapshot.industry} | **Generated by:** Omnia1 Growth OS v2`,
    "",
    "---",
    "",
    "## 📊 Performance Summary",
  ...
  lines.push("---", "", "## 🧪 Experiments Planned", "");
  for (const e of result.experiments) {
    lines.push(
      ...
      `**Setup:** ${e.setup}`,
      "",
      `**Success metrics:** ${e.successMetrics.join(" · ")}`,
      "",
  ...
  lines.push(
    ...
    "---",
    "",
    "## ⚠️ Risks + What We're Watching",
    "",
    `**Risk Score: ${result.risk.riskScore}/100 — ${riskLabel}**`,
    ""
  );
  ...
  lines.push(
    ...
    "---",
    "",
    `*Generated by AI Growth Operator | Omnia1 Growth OS v2 · ${date}*`
  );
        ...
        >
          <div>
            ...
            <h3 style={{ fontSize: "0.9375rem", fontWeight: 700, color: "var(--fg1)", margin: 0 }}>
              {snapshot.brandName} — Weekly Growth Report
            </h3>
          </div>
